...
//onway
export const orderOnway = asyncHandler(async (req, res, next) => {
  const { orderId } = req.params;
  const order = await orderModel.findOne({
    _id: orderId,
    userId: req.user._id,
  ...
  if (order.status == "Placed" ) {
    ...
    return next(new Error("order not placed yet"), { cause: 400 });
  }
});

//rejected
export const orderRejected = asyncHandler(async (req, res, next) => {
  const { orderId } = req.params;
  const order = await orderModel.findOne({
    _id: orderId,
    userId: req.user._id,
  ...
  if (order.status == "Placed" || order.status == "WaitForpayment") {
    ...
    return next(new Error("you can't reject order"), { cause: 400 });
  }
})
